...
class LevelsMapper(Module):
    ...
    def __init__(self, opts):
        ...
        self.opts = opts
        self.attention = MultiheadAttention(embed_dim=opts.embedding_dim, num_heads=opts.num_heads)
    ...
    def forward(self, x):
       ...
       # Apply cross-attention
        x_coarse, _ = self.attention(x_coarse, x_medium, x_fine) # Use medium as keys and fine as values for coarse
        x_medium, _ = self.attention(x_medium, x_fine, x_coarse) # Use fine as keys and coarse as values for medium
        x_fine, _ = self.attention(x_fine, x_coarse, x_medium) # Use coarse as keys and medium as values for fine
